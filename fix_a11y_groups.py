...
# Fix Rede filters specifically:
# Replace `<label class="block mb-2 text-xs font-bold text-slate-500 uppercase">Rede</label>`
# with `<span id="main-rede-label" class="block mb-2 text-xs font-bold text-slate-500 uppercase">Rede</span>`
# and add `aria-labelledby="main-rede-label"` to the `role="group"` div
html_content = re.sub(
