...
CREATE TABLE IF NOT EXISTS public.config_city_branches (
    ...
    CONSTRAINT unique_cidade UNIQUE (cidade)
);
...
DROP POLICY IF EXISTS "Permitir leitura para todos" ON public.config_city_branches;
DROP POLICY IF EXISTS "Permitir escrita para admins" ON public.config_city_branches;
DROP POLICY IF EXISTS "Admin All Operations" ON public.config_city_branches;
DROP POLICY IF EXISTS "Authenticated Read Access" ON public.config_city_branches;
