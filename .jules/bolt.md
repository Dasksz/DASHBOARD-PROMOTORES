...
## 2026-04-01 - [Collapse .filter().map() Chains in Render Paths]
**Learning:** Chaining array methods like `.filter(...).map(...)` on large datasets (e.g. `rawData` in `updateLpView`) creates intermediate array allocations and causes the dataset to be iterated twice `O(2N)`. This causes memory bloat and garbage collection pauses during UI rendering.
**Action:** Refactor these chains into a single vanilla `for` loop that both filters and transforms the data in one `O(N)` pass, pushing the result into a pre-allocated array.
