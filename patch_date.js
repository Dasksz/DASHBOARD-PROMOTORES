...
const oldLogic = `            // Date Filter (Proportional)
...
const newLogic = `            // Date Filter (Proportional)
            // Assumes sales have DTPED (timestamp or date string)
            // selectedCoverageDateRange = { start: 'YYYY-MM-DD', end: 'YYYY-MM-DD' }
            if (!isExcluded('date') && selectedCoverageDateRange.start && selectedCoverageDateRange.end) {
...
if (content.includes(oldLogic)) {
    ...
    console.log("Old logic replaced successfully.");
} else {
    console.log("Could not find the exact old logic. Trying regex approach...");
}
