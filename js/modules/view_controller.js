import { state, ColumnarDataset, IndexMap, sanitizeData, initializeOptimizedDataStructures, calculateHistoricalBests } from './data.js';
import { normalizeKey, parseDate, formatDate, normalize, getFirstName, escapeHtml, debounce, runAsyncChunked } from './utils.js';
import { charts, initCharts, renderMetaRealizadoPosChart, renderMetaRealizadoChart } from './charts.js';
import { hierarchyState, userHierarchyContext, resolveUserContext, setupHierarchyFilters, updateHierarchyDropdown, getHierarchyFilteredClients } from './filters.js';
import { navigateTo, openOrderModal, initUI } from './ui.js';
import { FORBIDDEN_KEYS, MIX_SALTY_CATEGORIES, MIX_FOODS_CATEGORIES, SPECIAL_CLIENTS, SPECIAL_RCAS, QUARTERLY_DIVISOR } from './constants.js';

export function initViewLogic() {
        // const embeddedData = window.embeddedData; // NO LONGER USED locally, use state
        ...
        */
        ...
        const stockData08 = state.stockData08;
        const innovationsMonthData = state.innovationsMonthData;
        const clientMapForKPIs = state.clientMapForKPIs;
        const activeProductCodesFromCadastro = state.activeProductCodesFromCadastro;
        const productDetailsMap = state.productDetailsMap;
        const passedWorkingDaysCurrentMonth = state.passedWorkingDaysCurrentMonth || 1;
        let lastSaleDate = state.lastSaleDate;
        const clientsWithSalesThisMonth = state.clientsWithSalesThisMonth;
        const optimizedData = state.optimizedData;
        const clientLastBranch = state.clientLastBranch;
        const clientRamoMap = state.clientRamoMap;
        let maxWorkingDaysStock = state.maxWorkingDaysStock;
        let sortedWorkingDays = state.sortedWorkingDays;
        let customWorkingDaysStock = state.customWorkingDaysStock;
        ...
        if (state.clientCoordinates) {
            ...
            const coords = Array.isArray(state.clientCoordinates) ? state.clientCoordinates : Object.values(state.clientCoordinates);
            coords.forEach(c => {
...
const supervisorGroups = new Map();
    ...
                });
        ...
        // Initial Render Logic Removed (Handled by Main)
        ...
        if (importBtn && importModal) {
            ...
            // --- AI Insights Logic ---
                ...
                try {
                    ...
                    // 2. Call API
                    const metaEntry = state.metadata ? state.metadata.find(m => m.key === 'groq_api_key') : null;
