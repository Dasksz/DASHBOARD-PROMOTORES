                ...
                coverageTableBody.innerHTML = filteredTableData.slice(0, 500).map(item => {
                    ...
                    return `
                        <tr class="hover:bg-slate-700/50">
                            <td data-label="Produto" class="px-2 py-1.5 md:px-4 md:py-2 text-[10px] md:text-xs truncate max-w-[120px] md:max-w-xs" title="${window.escapeHtml(item.descricao)}">${window.escapeHtml(item.descricao)}</td>
        ...
        function calculateProductVariation(currentData, historyData) {
            ...
            const _isAltMode_6 = isAlternativeMode(selectedTiposVenda);
            historyData.forEach(item => {
                if (!_isAltMode_6 && item.TIPOVENDA !== '1' && item.TIPOVENDA !== '9') return;
