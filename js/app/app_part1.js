        ...
        function renderMetaRealizadoClientsTable(data, weeks) {
            ...
            if (pageData.length === 0) {
                ...
                const rowsHTML = pageData.map((row, i) => {
                    ...
                    // Mobile Content (Compact List)
                    const mobileContent = `
                        <div class="md:hidden w-full" onclick="openMetaRealizadoDetailsModal(${i}, 'client')">
                            <div class="flex justify-between items-center mb-1">
                                <span class="font-bold text-slate-200 text-sm truncate pr-2">${row.codcli || ''} - ${escapeHtml(row.razaoSocial || '')}</span>
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                                </svg>
                            </div>
