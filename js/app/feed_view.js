            ...
            if (input && window.flatpickr) {
                flatpickrInstance = window.flatpickr(input, {
                    ...
                    onReady: function(selectedDates, dateStr, instance) {
                        ...
                        btnContainer.innerHTML = `<button type="button" id="feed-flatpickr-filter-btn" class="bg-[#FF5E00] hover:bg-[#CC4A00] text-white font-bold py-1 px-4 rounded text-sm transition-colors focus-visible:ring-2 focus-visible:ring-[#FF5E00] focus-visible:outline-none focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900">Filtrar</button>`;
        ...
        data.forEach(visit => {
                ...
                if (fotos.length > 0) {
                    ...
                    fotos.forEach((foto, index) => {
                        ...
                        const locationBtnHtml = `
                            <button onclick="window.FeedVisitas.openLocationModal(${visit.id})" class="absolute top-2 right-2 z-20 bg-black/50 hover:bg-[#FF5E00] text-white p-1.5 rounded-full backdrop-blur-sm transition-colors border border-white/20 shadow-lg focus-visible:ring-2 focus-visible:ring-[#FF5E00] focus-visible:outline-none focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900" title="Ver Localização" aria-label="Ver Localização">
                        ...
                        try {
                            ...
                            originalUrl = url.split('?')[0] + '?width=800&quality=80';
                        }
                ...
                if (respostasObj && typeof respostasObj === 'object') {
                    ...
                    if (respostasFormatadas.length > 0) {
                        resumoRespostasHtml = `
                            <div class="mt-2">
                                <button onclick="window.FeedVisitas.toggleResumo(this, '${visit.id}')" class="text-sm text-slate-400 font-medium hover:text-slate-200 transition-colors cursor-pointer inline-flex items-center gap-1 focus-visible:ring-2 focus-visible:ring-[#FF5E00] focus-visible:outline-none focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900">
                                    <span>Mais</span>
                    ...
                    <div class="px-4 pt-3 pb-4 flex flex-col gap-2">
                        ...
                        <div class="flex items-center justify-between w-full h-8">
                            ...
                            <div class="flex-1 flex justify-start">
                                ...
                                <button onclick="window.FeedVisitas.toggleFavorite('${visit.id}', this)" class="p-1 rounded-full transition-all hover:bg-slate-800/50 ${(visit.favoritado_por || []).includes(window.userId) ? 'text-yellow-400 scale-110' : 'text-slate-400 hover:text-white'} focus-visible:ring-2 focus-visible:ring-[#FF5E00] focus-visible:outline-none focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900" title="Favoritar" aria-label="Favoritar visita">
